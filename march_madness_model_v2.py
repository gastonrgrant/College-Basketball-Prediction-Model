...
# =========================================================
# MATCHUP MODEL — V2 HEURISTIC ENGINE
# =========================================================
# IMPORTANT:
# This is still a heuristic model, not a trained ML model.
# The structure below intentionally separates:
#   1) feature creation
#   2) margin prediction
#   3) probability conversion
# so historical training can replace steps 2 and 3 later without
# rewriting the rest of the program.


def predict_stat(off_value, def_allowed_value, offense_weight=0.50):
    """Blend a team's offensive stat with what the opponent allows.

    offense_weight is intentionally exposed so it can later be learned
    from historical games instead of hard-coded at 50/50.
    """
    defense_weight = 1.0 - offense_weight
    return offense_weight * off_value + defense_weight * def_allowed_value
...
def build_matchup_features(A, B, baselines):
    """Create one symmetric set of Team A minus Team B matchup features.

    Positive values always favor Team A.
    Keeping feature construction separate is the key step that will let us
    feed these exact features into a trained regression/boosting model later.
    """

    # Match each offense against what the opposing defense normally allows.
    pred_A_ortg = predict_stat(A["ORtg"], B["Opp_ORtg"])
    pred_B_ortg = predict_stat(B["ORtg"], A["Opp_ORtg"])

    pred_A_efg = predict_stat(A["eFG%"], B["Opp_eFG%"])
    pred_B_efg = predict_stat(B["eFG%"], A["Opp_eFG%"])

    pred_A_tov = predict_stat(A["TOV%"], B["Opp_TOV%"])
    pred_B_tov = predict_stat(B["TOV%"], A["Opp_TOV%"])

    pred_A_orb = predict_stat(A["ORB%"], B["Opp_ORB%"])
    pred_B_orb = predict_stat(B["ORB%"], A["Opp_ORB%"])

    pred_A_ftr = predict_stat(A["FT/FGA"], B["Opp_FT/FGA"])
    pred_B_ftr = predict_stat(B["FT/FGA"], A["Opp_FT/FGA"])

    # Standardized matchup edges. Positive ALWAYS means Team A advantage.
    ortg_edge = (
        zscore(pred_A_ortg, "ORtg", baselines)
        - zscore(pred_B_ortg, "ORtg", baselines)
    )
    efg_edge = (
        zscore(pred_A_efg, "eFG%", baselines)
        - zscore(pred_B_efg, "eFG%", baselines)
    )

    # Lower offensive turnover rate is better, so reverse the subtraction.
    tov_edge = (
        zscore(pred_B_tov, "TOV%", baselines)
        - zscore(pred_A_tov, "TOV%", baselines)
    )
    orb_edge = (
        zscore(pred_A_orb, "ORB%", baselines)
        - zscore(pred_B_orb, "ORB%", baselines)
    )
    ftr_edge = (
        zscore(pred_A_ftr, "FT/FGA", baselines)
        - zscore(pred_B_ftr, "FT/FGA", baselines)
    )

    srs_edge_raw = A["SRS"] - B["SRS"] if "SRS" in A.index else 0.0
    srs_edge_std = (
        zscore(A["SRS"], "SRS", baselines)
        - zscore(B["SRS"], "SRS", baselines)
        if "SRS" in A.index else 0.0
    )

    sos_edge_std = (
        zscore(A["SOS"], "SOS", baselines)
        - zscore(B["SOS"], "SOS", baselines)
        if "SOS" in A.index else 0.0
    )

    # Dean-Oliver-style Four Factors structure.
    # Unlike the old version, ORtg is NOT counted as one of the four factors.
    # These are temporary heuristic weights; historical training will replace them.
    four_factor_score = (
        0.40 * efg_edge
        + 0.25 * tov_edge
        + 0.20 * orb_edge
    ...
    return {
        ...
        "srs_edge_raw": srs_edge_raw,
        "srs_edge_std": srs_edge_std,
        "sos_edge_std": sos_edge_std,
    }


def heuristic_projected_margin(features, expected_possessions, baselines):
    """Temporary V2 margin model until historical training replaces it.

    Every component is first put into point-margin-like units. This fixes the
    old behavior where a rich factor score was calculated but ignored.
    """

    # Efficiency edge becomes expected points at this game's possession count.
    efficiency_margin = (
        (features["pred_A_ortg"] - features["pred_B_ortg"])
        * expected_possessions / 100.0
    )

    # SRS is already approximately expressed in point-differential units.
    srs_margin = features["srs_edge_raw"]

    # Convert the standardized Four Factors score to a season-relative point scale.
    # This is deliberately modest so Four Factors refine rather than dominate.
    srs_std = baselines.get("SRS", {}).get("std", 10.0)
    four_factor_margin = features["four_factor_score"] * (0.50 * srs_std)

    # TEMPORARY heuristic blend. These are the next numbers we will replace
    # with coefficients learned from historical games.
    projected_margin = (
        0.55 * efficiency_margin
        + 0.35 * srs_margin
        + 0.10 * four_factor_margin
    )

    return projected_margin, {
        "efficiency_margin": efficiency_margin,
        "srs_margin": srs_margin,
        "four_factor_margin": four_factor_margin,
    }


def margin_to_win_probability(projected_margin, scale=6.5):
    """Convert Team A projected margin to win probability.

    scale=6.5 is retained TEMPORARILY from V1. Once historical game data is
    added, this function should be calibrated from observed win rates.
    """
    return 1.0 / (1.0 + np.exp(-projected_margin / scale))
...
def compute_matchup(team_a, team_b, full_df, baselines, percentile_tables,
    ...
    # Composite score is now meaningful and uses the same ingredients as the
    # margin model. Positive favors Team A; negative favors Team B.
    model_score = (
        0.55 * features["ortg_edge"]
        + 0.35 * features["srs_edge_std"]
        + 0.10 * features["four_factor_score"]
    )
    ...
    return {
        ...
        "raw_edges": {
            "Efficiency": features["ortg_edge"],
            "Shooting": features["efg_edge"],
            "Turnovers": features["tov_edge"],
            "Rebounding": features["orb_edge"],
            "Free Throw Pressure": features["ftr_edge"],
            "Four Factors": features["four_factor_score"],
            "SRS": features["srs_edge_std"],
            "SOS": features["sos_edge_std"],
        },
